...
U.safeHtml = function (html) {
  ...
  /* Where the tag ends: the first > that is not inside a quoted attribute.
     Taking the first > full stop spilled title="a > b" into the script as
     text — the same leak as before, arriving from the other direction. An
     unbalanced quote falls back to the plain first >, so a stray " in prose
     cannot swallow the rest of the paragraph. */
  ...
  while (i < src.length) {
    ...
    var gt = tagEnd(src, lt + 1);
    if (gt < 0) gt = src.indexOf('>', lt);
